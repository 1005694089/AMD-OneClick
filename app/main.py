...
def _catalog_image_public(record: dict) -> dict:
    """Strip internal fields from a catalog image row for NON-admin responses.

    source_ref/source_type expose upstream registry topology (private ACR account names, internal
    hosts) and sync_message can carry internal detail — none of which should reach anonymous/
    non-admin catalog consumers. Keep only what the launch UI needs."""
    if not record:
        return {}
    return {
        "id": record.get("id"),
...
def _prepare_custom_image_for_launch(user: dict, image: str) -> Optional[str]:
    ...
    if row["build_status"] == "evicted":
        # Requeueing an evicted image triggers a real Dockerfile rebuild. Honor the
        # USER_CUSTOM_BUILDS_ENABLED kill-switch here too (build_custom_image guards creation, but a
        # relaunch of a previously-evicted image would otherwise rebuild, bypassing the switch).
        if not settings.USER_CUSTOM_BUILDS_ENABLED:
            raise HTTPException(
                status_code=403,
                detail="Custom image builds are disabled; this image can no longer be rebuilt.")
...
async def _run_image_op(func, *args):
    loop = asyncio.get_running_loop()
    return await loop.run_in_executor(_preheat_executor, func, *args)
...
# unlike a lock whose acquisition is deferred into a background task. The background task discards
# the key when done, so the set self-bounds (no unbounded lock-dict growth).
_image_seq_inflight: set[str] = set()


# Short-TTL cache for the pod-status read behind the friendly-503 error page. A single browser page
# load of a down instance fires many concurrent sub-resource requests, each landing on the error
# path; without this each would issue its own apiserver read, amplifying load exactly during an
# outage. Cache the (details, read_failed) result per instance for a few seconds so a burst collapses
# to one read. Value: (expiry_monotonic, details_or_None, read_failed).
_POD_STATUS_ERROR_CACHE_TTL = 3.0
_pod_status_error_cache: dict[str, tuple] = {}
# Per-instance locks so concurrent cache MISSES coalesce to a single apiserver read (a page load
# fires many sub-resource requests at once; without this each would issue its own read within the
# in-flight window, defeating the cache's purpose during an outage).
_pod_status_error_locks: dict[str, asyncio.Lock] = {}


async def _cached_pod_status_for_error_page(instance_id: str):
    now = time.monotonic()
    cached = _pod_status_error_cache.get(instance_id)
    if cached and cached[0] > now:
        return cached[1], cached[2]
    lock = _pod_status_error_locks.get(instance_id)
    if lock is None:
        lock = asyncio.Lock()
        _pod_status_error_locks[instance_id] = lock
    async with lock:
        # Re-check: another coroutine may have populated the cache while we waited for the lock.
        now = time.monotonic()
        cached = _pod_status_error_cache.get(instance_id)
        if cached and cached[0] > now:
            return cached[1], cached[2]
        ...
        try:
            details = await asyncio.to_thread(
                k8s_client.get_pod_status_details, "", instance_id)
        except Exception:
            details, read_failed = None, True
        _pod_status_error_cache[instance_id] = (now + _POD_STATUS_ERROR_CACHE_TTL, details, read_failed)
    # Opportunistic prune so the dicts can't grow unbounded across many instance ids.
    if len(_pod_status_error_cache) > 512:
        for k, v in list(_pod_status_error_cache.items()):
            if v[0] <= now:
                _pod_status_error_cache.pop(k, None)
                _pod_status_error_locks.pop(k, None)
    return details, read_failed
...
# friendly 503 page instead of a bare 500. It does NOT include httpx.HTTPStatusError, so a
# legitimate upstream 4xx/5xx still passes through untouched.
# NOTE: this only covers failures during client.send() (connect + headers). A failure DURING body
# streaming (aiter_raw) — e.g. the pod is OOMKilled mid-response — cannot be converted to a 503
# because the 200/206 status+headers have already been flushed to the client; the connection is
# simply aborted. That's inherent to streaming, not something this tuple can catch.
_PROXY_UNAVAILABLE_ERRORS = (httpx.TransportError,)


async def _friendly_unavailable_response(instance_id: str, exc: Exception) -> Response:
    """Map an upstream connection failure to a friendly 503 page instead of a bare 500.

    Inspects the pod so the message distinguishes "still starting" (pending/ContainerCreating)
    from "failed to start" (ImagePullBackOff/CrashLoop). Falls back to a generic starting message
    if the pod can't be read. The pod read is a blocking k8s call → dispatched via to_thread so it
    can't stall the single uvicorn worker (this path fires on every failed proxy poll)."""
    title = "Instance unavailable"
    detail = "The instance is not reachable right now. Please try again in a moment."
    retry = True
    details, read_failed = await _cached_pod_status_for_error_page(instance_id)
    # details is None specifically when the pod no longer exists (get_pod_status_details returns
    # None on a 404). Distinguish that PERMANENTLY-gone case from a transient read failure so we
    # don't auto-refresh forever on a deleted instance.
    if details is None and not read_failed:
        title = "Instance no longer exists"
        detail = "This instance has been deleted. Return to your dashboard to start a new one."
        retry = False
        logger.warning("Proxy target pod gone for %s: %s", instance_id, exc)
        meta_refresh = ""
        html = (
            f"<!doctype html><html><head><meta charset='utf-8'>{meta_refresh}"
            f"<title>{title}</title></head><body style='font-family:system-ui,sans-serif;"
            f"max-width:640px;margin:80px auto;text-align:center;color:#333'>"
    ...
    if status in {"pending", "jupyter_starting"} or reason in {"ContainerCreating", "PodInitializing"}:
        title = "Instance is still starting"
        detail = "Your instance is starting up. This page will retry automatically."
    elif "imagepull" in reason.lower() or "errimagepull" in reason.lower():
        title = "Instance failed to start"
        detail = "The instance image could not be pulled. Please contact an administrator."
        retry = False
    elif "crashloop" in reason.lower():
        title = "Instance failed to start"
        detail = "The instance container is crashing on startup. Please contact an administrator."
        retry = False
    logger.warning("Proxy upstream unavailable for %s (%s): %s", instance_id, status or "unknown", exc)
    meta_refresh = '<meta http-equiv="refresh" content="5">' if retry else ""
    html = (
        f"<!doctype html><html><head><meta charset='utf-8'>{meta_refresh}"
        f"<title>{title}</title></head><body style='font-family:system-ui,sans-serif;"
        f"max-width:640px;margin:80px auto;text-align:center;color:#333'>"
...
async def index(request: Request):
    ...
    return templates.TemplateResponse(
        ...
        "index.html",
        {
            ...
            "image_catalog_json": json.dumps([_catalog_image_public(img) for img in images]),
...
async def build_custom_image(req: CustomImageBuildRequest, user: dict = Depends(current_user)):
    if not settings.USER_CUSTOM_BUILDS_ENABLED:
        raise HTTPException(
            status_code=403,
            detail="Custom image builds are disabled. Use an image from the admin-curated catalog.",
        )
...
async def profile_list_templates(user: dict = Depends(current_user)):
    return {
        ...
        "images": [_catalog_image_public(img) for img in list_images(enabled_only=True)],
    }
